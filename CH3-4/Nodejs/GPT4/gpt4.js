...
//抽象摘要 prompt
//requestModel.temperature = 0.7;
// requestModel.messages = [
//   { role: 'system', content: '現在開始你是一位文字閱讀高手。' },
//   { role: 'user', content: `請為以下文章生成使用繁體中文輸出100個字以內的抽象摘要，摘要包含其主要觀點，### 微軟日前已宣布將 GPT-4 導入全新 Bing 搜尋引擎和 Microsoft 365 Copilot，今日的發表將有助於各產業，同樣以先進模型為基礎，藉由 Azure OpenAI 服務發展自身的應用程式。 
...
//提取摘要 prompt
requestModel.temperature = 0.3;
requestModel.messages = [
  { role: 'system', content: '現在開始你是一位文字閱讀高手。' },
  { role: 'user', content: `請為以下文章生成使用繁體中文輸出提取摘要，摘要包含3至5個重點句子，### 微軟日前已宣布將 GPT-4 導入全新 Bing 搜尋引擎和 Microsoft 365 Copilot，今日的發表將有助於各產業，同樣以先進模型為基礎，藉由 Azure OpenAI 服務發展自身的應用程式。 
